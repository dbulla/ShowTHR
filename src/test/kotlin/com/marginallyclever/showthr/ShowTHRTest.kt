...
//        val input = listOf(ThetaRho(0.0, 1.0), ThetaRho(2.0, 1.0))
//        val result = ShowTHR.expandSequence(input)
//        assertTrue(result.size > input.size, "Result should contain more points after expansion")
//        assertEquals(ThetaRho(0.0, 1.0), result.first(), "First point should remain unchanged")
//        assertEquals(ThetaRho(2.0, 1.0), result.last(), "Last point should remain unchanged")
//        result.forEachIndexed { index, point ->
//            if (index > 0) {
//                val prevPoint = result[index - 1]
//                val deltaTheta = point.theta - prevPoint.theta
//                val deltaRho = point.rho - prevPoint.rho
...
//        val input = listOf(ThetaRho(0.0, 0.0), ThetaRho(3.14, 0.0))
//        val result = ShowTHR.expandSequence(input)
//        assertTrue(result.size > input.size, "Result should contain more points after expansion")
//        assertEquals(ThetaRho(0.0, 0.0), result.first(), "First point should remain unchanged")
//        assertEquals(ThetaRho(3.14, 0.0), result.last(), "Last point should remain unchanged")
//    }
}